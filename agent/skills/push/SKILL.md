...
> Windows safety: never invoke the system `bash.exe`; that launches WSL.
> This optional wrapper may run only with the explicitly resolved Git for
> Windows `bash.exe`. If Git Bash is unavailable, reproduce these pre-flight,
> approval, and audit guarantees with native PowerShell and `git`.
