...
async function cmdPropose(
	...
	// Keep the canonical ISO timestamp in the patch header, but make the ID safe
	// for filenames on every supported platform (notably Windows, where ':' is
	// reserved).
	const draftId = `${timestamp.replace(/:/g, "-")}-${slug}`;
