...
const portablePath = (...parts: string[]): string => parts.join("/");
...
async function checkCommand(
	...
	return {
		exitCode: run.verdict === "pass" ? 0 : 1,
		stdout: `evalfly check ${run.run_id}: ${run.verdict}\nreport: ${run.context?.eval_report_path ?? portablePath("evals", "reports", `${run.run_id}.md`)}\n`,
		stderr: "",
	};
}
...
async function reportCommand(
	...
	if (!result.ok) {
		throw new Error(
			`invalid ${portablePath("evals", "runs", `${runId}.json`)}:\n${result.errors.join("\n")}`,
		);
	}
	...
	return {
		exitCode: 0,
		stdout: `evalfly report written: ${portablePath("evals", "reports", `${runId}.md`)}\n`,
		stderr: "",
	};
}
...
async function tracesCommand(cwd: string): Promise<DispatchResult> {
	...
	for (const file of files) {
		...
		const relativeTracePath = portablePath("evals", "traces", "sanitized", file);
...
async function readSavedRuns(cwd: string): Promise<EvalRun[]> {
	...
	for (const file of files) {
		...
		if (!result.ok) {
			throw new Error(
				`invalid ${portablePath("evals", "runs", file)}:\n${result.errors.join("\n")}`,
			);
		}
		assertSafeRunId(result.value.run_id);
		if (result.value.run_id !== runId) {
...
async function canonicalReportPath(
	...
	const relativeReportPath = portablePath("evals", "reports", `${runId}.md`);
	try {
...
async function curateTraceCommand(
	...
	return {
		exitCode: 0,
		stdout: `evalfly trace curated: ${portablePath("evals", "traces", "sanitized", sanitizedName)}\n`,
		stderr: "",
	};
}
...
async function writeSanitizedTrace(
	...
	return {
		exitCode: 0,
		stdout: `evalfly trace ${action}: ${portablePath("evals", "traces", "sanitized", sanitizedName)}\n`,
		stderr: "",
	};
}
...
async function buildRunContext(
	...
	return {
		...
		eval_report_path: portablePath("evals", "reports", `${runId}.md`),
	};
}
...
async function writeNewArtifactFile(
	...
	try {
		...
		throw new Error(
			`${label} artifact already exists: ${portablePath(...artifactDirParts, `${runId}${extension}`)}`,
		);
	} catch (error) {
		if (!missingPathError(error)) {
			throw error;
...
async function replaceArtifactFile(
	...
	const relativeTargetPath = portablePath(...artifactDirParts, `${runId}${extension}`);
	try {
...
function renderReport(run: RunRecord): string {
	...
	const lines = [
		...
		`evalReportPath: ${run.context?.eval_report_path ?? portablePath("evals", "reports", `${run.run_id}.md`)}`,
		"",
		"## Results",
	];
...
async function boundedPath(
	...
	if (realBasePath !== expectedBasePath) {
		throw new Error(
			`unsafe raw trace directory: ${portablePath(...baseParts)} (raw trace directory must be ${portablePath(...baseParts)})`,
		);
	}
...
async function assertExactArtifactDir(
	...
	if (realArtifactDir !== expectedArtifactDir) {
		throw new Error(
			`unsafe artifact directory: ${portablePath(...artifactDirParts)} (artifact directory must be ${portablePath(...artifactDirParts)})`,
		);
	}
}
...
async function assertArtifactDir(
	...
	if (
		...
		throw new Error(
			`unsafe artifact directory: ${portablePath(...artifactDirParts)} (artifact directory must stay within cwd)`,
		);
	}
	const expectedArtifactDir = resolve(realCwd, ...artifactDirParts);
	if (realArtifactDir !== expectedArtifactDir) {
